...
Después de ejecutar el script de creación, se crea un usuario administrador con las siguientes credenciales:
...
Primero ejecuta las migraciones de Prisma para actualizar el modelo User:
...
const newUserResponse = await fetch('http://localhost:3010/api/users', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${token}`,
    'Content-Type': 'application/json'
